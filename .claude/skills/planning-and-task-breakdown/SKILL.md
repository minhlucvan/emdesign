...
In this repo (Mezon Mentor Bot / "MeKnow"), the plan lives in the OpenSpec change: architectural decisions go in `openspec/changes/cNNNN-<slug>/design.md`, and the task list goes in `tasks.md`. Both are generated by `/opsx:propose` and refined here; `/opsx:ship-plan` groups them into test-first work-units and you tick `tasks.md` off during `/opsx:ship-code`.
...
- The plan is OpenSpec: decisions go in `openspec/changes/cNNNN-<slug>/design.md`, the
  ...
  here; `/opsx:ship-plan` groups them into work-units and you tick `tasks.md` during
  `/opsx:ship-code`. This skill is the PLAN+TASKS half of `spec-driven-development`.
