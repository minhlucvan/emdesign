...
function CardItem({ variant, title, price, description, features }: CardEntry) {
...
export interface OverviewCardExamplesProps {
  className?: string;
}

/**
 * OverviewCardExamples — Stripe-inspired card variants section.
 * Uses inline styles to match reference-example.html section#cards CSS.
 * Renders pricing-light, pricing-featured, and feature-alt card variants in a responsive grid.
 */
export function OverviewCardExamples({ className = '' }: OverviewCardExamplesProps) {
  return (
    <section id="cards" className={className} style={s.section}>
      <style>{`#cards .card-grid li::before{content:"✓ ";font-weight:700;color:var(--check-color,#533afd)}`}</style>
