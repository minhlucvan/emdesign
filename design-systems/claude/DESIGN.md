...
The spacing scale is defined in the `spacing:` YAML block and is built on a 4px base unit:

| Token | Value | Use |
|---|---|---|
| `{spacing.xxs}` | 4px | Icon-to-text gaps, badge interior padding, tiny inline adjustments |
| `{spacing.xs}` | 8px | Tab padding, small element clusters, radio/checkbox-to-label |
| `{spacing.sm}` | 12px | Button padding (horizontal), input-to-icon, compact card groups |
| `{spacing.md}` | 16px | Standard element-to-element gap, form field stacking, nav-item spacing |
| `{spacing.lg}` | 24px | Card internal padding (code windows, connectors), list-item groups |
| `{spacing.xl}` | 32px | Feature card padding, model comparison cards, pricing tier cards |
| `{spacing.xxl}` | 48px | CTA band internal padding, coral callout card interior |
| `{spacing.section}` | 96px | Vertical rhythm between major page bands |

### Principles
Spacing favors generosity over density. Internal card padding starts at `{spacing.lg}` (24px) and goes up; `{spacing.xl}` (32px) is the default for content cards. The `{spacing.section}` (96px) rhythm gives the cream canvas room to breathe between bands -- Claude pages never feel cramped. On mobile, section rhythm halves to 48px and card padding drops to `{spacing.lg}` while maintaining the same proportional hierarchy. Gaps between elements (button rows, form clusters) use `{spacing.sm}` (12px) or `{spacing.md}` (16px) -- never less than 8px, keeping the warm editorial feel from feeling like a dense dashboard.

## Motion

### Duration & Easing
Claude.com uses motion sparingly -- the brand is editorial, not theatrical. When motion does occur, it favors **slow entrances** with long durations (300-500ms) and **decelerated easing** (ease-out, never ease-in or bounce).

| Token | Duration | Easing | Use |
|---|---|---|---|
| Fade in | 400ms | ease-out | Hero headline entrance on page load, card reveal on scroll |
| Fade out | 200ms | ease-out | Dismissal of cookie consent card, tooltips |
| Slide up | 350ms | ease-out | Feature cards entering viewport on scroll |
| Hover lift | 200ms | ease-out | Subtle shadow elevation on hoverable cards |
| Color transition | 200ms | ease-out | Button background shifts (`{colors.primary}` to `{colors.primary-active}`) |

### Affordances
- **Scroll-triggered reveals** use IntersectionObserver with a 150px root margin -- cards fade + slide 24px upward over 350ms. The staggered delay between sibling cards is 80ms (short enough to feel orchestrated, long enough to read as sequential).
- **Anchor link smooth scroll** uses 400ms ease-out -- fast enough to feel responsive, slow enough to orient the reader.
- **The hero code-window typewriter effect** (homepage): characters appear at 40-60ms intervals with a subtle opacity fade (not a cursor-blinking reveal). This is the system's most elaborate animation and is reserved for the one hero slot.
- **No parallax, no horizontal scroll-triggered transforms, no orbital rotations.** Claude's motion vocabulary stays in the fade/slide family.

### Reduced Motion
All animations respect `prefers-reduced-motion: reduce` by presenting the final state immediately. The hero code-window typewriter shows all text at once.

## Voice

### Brand Voice
Claude.com's written voice matches its visual voice: **warm, direct, editorial, and understated**. The copy reads like a thoughtful colleague explaining something neat -- never like a marketing landing page in the AI-arms-race register.

| Quality | What it means | Example (paraphrased) |
|---|---|---|
| Warm | Second-person ("you") throughout. Approachable without being casual. Informative without being technical. | "Meet your thinking partner." -- not "Introducing the next-generation AI assistant." |
| Direct | Short sentences, active voice. No jargon. No feature laundry lists. | "Claude can read and write long documents." -- not "Our platform leverages extended-context natural language processing." |
| Editorial | Headlines carry serif authority. Body copy stays lean. The system trusts the reader's intelligence. | Section heads are one-line theses ("Built for work", "Trusted by researchers"), not keyword-stuffed SEO headings. |
| Understated | No superlatives ("best", "revolutionary", "industry-leading"). Claims are specific and demonstrable. | "Claude can process up to 200K tokens -- roughly the length of a novel." -- not "Unprecedented context window capacity." |

### Tone by Surface
- **Product marketing pages** (homepage, solutions, pricing): Warm and explanatory. Still direct, but uses display serif headlines to convey gravitas. CTAs are "Try Claude" -- not "Get Started Free" or "Sign Up Now".
- **Developer docs / Code product pages** (Claude Code, API): More technical but still warm. Uses `{component.code-window-card}` to show real output. No developer-targeting copy ever talks down.
- **Company / Research** (about, research posts): Most editorial register. Longer paragraphs, full citations. The serif display gets the heaviest use here.

### Do
- Lead with what the user can do, not what the product is.
- Use contractions ("you'll", "it's", "Claude's") -- they read as warm, not casual.
- Keep button labels short: "Try Claude", "Learn more", "Sign in", "Get started". Never "Submit", never "Click here".
- Refer to the company as "Anthropic" in footer and legal contexts; refer to the product as "Claude" everywhere else.

### Don't
- Don't use exclamation marks in headlines. The brand's conviction is declarative, not excited.
- Don't compare directly with competitors by name. Claude's positioning is "what you can do with Claude" rather than "why Claude is better than X".
- Don't write "AI" or "LLM" in consumer-facing headlines. Use "Claude" or the specific capability.
- Don't use emoji in marketing copy. (Product UI inside mockups may use system emoji in chat bubbles.)

## Anti-patterns

### Visual Anti-patterns
- **Cool gray or pure white canvas.** The cream tint (`{colors.canvas}` -- #faf9f5) is the single most recognizable brand signal. A page on #ffffff or #f5f5f5 loses the brand in one decision.
- **Bold serif display copy.** Copernicus at weight 400 is deliberate and confident. Weight 700 reads as shouting -- the literary editorial quality is lost.
- **Coral overuse.** Coral is the brand voltage. Painting every button, badge, border, and icon coral turns the impact into noise. Coral stays scarce on individual elements and generous only on full-bleed callout cards.
- **Sans-serif display headlines.** Substituting StyreneB or Inter for Copernicus at h1/h2 makes Claude feel like every other AI SaaS tool. The serif display is non-negotiable.
- **Flat adjacent bands.** Two cream-on-cream bands in a row read as one band. The system rhythm alternates: cream → cream-card → dark-mockup → cream → coral-callout → dark-footer.
- **Shadows on cream cards.** The brand's elevation model is color-block-first. A drop shadow on a cream feature card reads as a different design system (Material, Bootstrap) entirely.

### Content Anti-patterns
- **Superlative marketing copy.** "Best", "revolutionary", "industry-leading" contradict the understated editorial voice.
- **Exclamation-heavy headlines.** Claude copy states facts with conviction; it doesn't shout.
- **Competitor naming.** The system positions Claude on its own merits, not by comparison.
- **Emoji in marketing copy.** Emoji belong in product chat bubbles, not in headlines or body copy.

### Component Anti-patterns
- **Coral `{component.button-primary}` on a coral `{component.callout-card-coral}` background.** The CTA inside a coral card inverts to a cream button; coral-on-coral loses the affordance.
- **Light `{component.button-secondary}` on a cream `{component.feature-card}`.** The hairline border disappears against the cream surface. Use the hairline + `{colors.surface-soft}` background variant or switch to `{component.button-primary}`.
- **Inline code in display headlines.** `{typography.code}` (JetBrains Mono) belongs in `{component.code-window-card}` and terminal mockups -- never in a Copernicus serif h1.
- **More than one `{component.code-window-card}` on a page.** The code-window typewriter effect is a hero moment; repeating it dilutes the signature.
