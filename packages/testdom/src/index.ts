...
export interface DomViolation {
  ...
  text?: string;           // text content snippet
  rule: string;            // e.g. "token-binding", "anti-pattern"
...
export function checkTokenBinding(declaredTokens: Record<string, string>): DomRuleResult {
...
export function checkAntiPatterns(declaredTokens: Record<string, string>): DomRuleResult {
  const violations: DomViolation[] = [];
  const allElements = document.querySelectorAll('*');
...
export function checkSpacing(spacingScale?: string[]): DomRuleResult {
  const violations: DomViolation[] = [];
  ...
  const scalePx = spacingScale.map(s => parsePx(s)).filter(Boolean) as number[];
  if (scalePx.length === 0) return { passed: true, score: 1, violations: [] };

  const allElements = document.querySelectorAll('*');

  for (const el of allElements) {
    if (el.tagName === 'SCRIPT' || el.tagName === 'STYLE' || el.tagName === 'BODY' || el.tagName === 'HTML') continue;
    const cs = getComputedStyle(el);

    for (const prop of ['marginTop', 'marginBottom', 'marginLeft', 'marginRight', 'paddingTop', 'paddingBottom', 'paddingLeft', 'paddingRight', 'gap'] as const) {
...
export function checkContrast(minContrast = 4.5): DomRuleResult {
  const violations: DomViolation[] = [];
  const allElements = document.querySelectorAll('*');
  ...
  for (const el of allElements) {
    ...
    if (!fg || !bg || bg === 'rgba(0, 0, 0, 0)' || fg === bg) continue;
...
export function evaluateAll(input: EvaluateRulesInput): EvaluateRulesResult {
  return {
...
// ═══════════════════════════════════════════════════════════════════════
// Browser bridge — attaches to window.__emdesign
// ═══════════════════════════════════════════════════════════════════════
...
function findNearestToken(hex: string, tokens: Record<string, string>): string | null {
