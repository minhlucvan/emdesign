...
import type { EvaluateRulesInput, EvaluateRulesResult } from './index.js';
...
export interface PageReport extends EvaluateRulesResult {
  summary: string;
}

// ── Clean API ─────────────────────────────────────────────────────

const TESTDOM_BUNDLE_CACHE: string | null = null;

function loadTestdomBundle(): string {
...
export async function evaluatePage(
  ...
  opts: { url?: string; spacingScale?: string[]; minContrast?: number } = {},
): Promise<PageReport> {
  ...
  const input: EvaluateRulesInput = {
    declaredTokens: tokens,
    spacingScale: opts.spacingScale,
    minContrast: opts.minContrast,
  };
  const result: EvaluateRulesResult = await page.evaluate(
    (inp: EvaluateRulesInput) => (window as any).__emdesign.evaluateRules(inp),
    input,
  );
...
function generateSummary(result: EvaluateRulesResult): string {
  const lines: string[] = [];
  const checks: [string, string, { passed: boolean; score: number; violations: any[] }][] = [
