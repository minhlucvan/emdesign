...
import { extractProject } from './project/extract.js';
...
export class WorkflowStore {
    ...
    updateStage(id, name, status, progress, error, extra) {
        ...
        if (extra?.agentOutput) {
            if (!session.agentOutput)
                session.agentOutput = [];
            session.agentOutput.push(...extra.agentOutput);
            if (session.agentOutput.length > 200)
                session.agentOutput = session.agentOutput.slice(-200);
        }
        // Update overall session status
        if (status === 'error')
            session.status = 'failed';
        else if (session.stages.every(s => s.status === 'done'))
            session.status = 'completed';
        this.save(id);
    }
    pushAgentOutput(id, text, timestamp) {
        const session = this.sessions.get(id);
        if (!session)
            return;
        if (!session.agentOutput)
    ...
    loadAll() {
        try {
            ...
            for (const file of files) {
                try {
                    const data = JSON.parse(fs.readFileSync(path.join(this.persistRoot, file), 'utf8'));
...
function parseDesignMdTokens(md) {
    const out = new Map();
...
function synthesizeDesignMd(name, extraction) {
...
function buildProjectTokensCss(overrides) {
...
function parseTokensCssMap(css) {
    const map = new Map();
...
function generateBuildSkill(name, dsDir) {
    ...
    for (const role of tokenOrder) {
        ...
        tokenRows.push(`| \`${role}\` | \`${twClass}\` | \`var(--${role})\` | ${usage[role] ?? '—'} (\`${value}\`) |`);
    }
...
function generateTasteSkill(name, dsDir) {
    const designMd = fs.readFileSync(path.join(dsDir, 'DESIGN.md'), 'utf8');
...
export class WorkflowOrchestrator {
    ...
    constructor(storeOrOptions, options) {
        if (storeOrOptions instanceof WorkflowStore) {
            ...
            this.options = { timeout: storeOrOptions?.timeout ?? 120_000 };
        }
    }
    /** Run the create-from-prompt workflow stages. */
    async runFromPrompt(input) {
        ...
        try {
            ...
            await this.runStage(sessionId, 'extract-tokens', 25, () => {
                ...
                if (fmMatch) {
                    ...
                    if (lines.length > 0)
                        artifacts['extracted-tokens'] = `${lines.length} tokens extracted`;
                }
            });
            ...
            await this.runStage(sessionId, 'validate', 100, () => {
                ...
                if (!v.ok)
                    throw new Error(`Design system validation failed: ${v.note}`);
            });
    ...
    async runStage(sessionId, name, progress, fn) {
        const session = this.store.get(sessionId);
     ...
     */
    async runFromProject(sessionId, input) {
        const stages = PROJECT_STAGE_NAMES.map((name) => ({
            name,
            status: 'pending',
            progress: 0,
        }));
        ...
        let canonical = false;
        let tokensCss = '';
        let currentStage = 'scan';
        const stage = async (n, progress, fn) => {
            currentStage = n;
            ...
            await stage('tokens', 45, () => {
                ...
                for (const r of extraction.proposedRoles) {
                    if (r.source === 'extracted' && r.evidence.length) {
                        ...
                        notes.push(`--${r.role} could not be confidently inferred; using documented default ${r.evidence[0]?.value}.`);
                    }
                }
                if (canonical) {
...
export class SessionQueue {
    ...
    enqueue(sessionId, metadata) {
        ...
        if (!session)
            return;
        if (metadata)
            this.metaMap.set(sessionId, metadata);
    ...
    get activeCount() { return this.queue.length + this.running.size; }
    drain() {
